              ...
              <div className="p-5 flex flex-col sm:flex-row gap-5">
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-5 gap-2" style={{ minWidth: 300 }}>
                    {[
                      { label: "Download Speed", val: latest.download, unit: "Mbps", icon: Download, cls: ragDl(latest.download), bg: "bg-emerald-50", prev: prev?.download, higher: true, rgb: "34,211,238", ctx: `Download ${fmt(latest.download)}Mbps, Upload ${fmt(latest.upload)}Mbps, Latency ${fmt(latest.latency)}ms, Jitter ${fmt(latest.jitter)}ms, Packet Loss ${fmt(latest.packetLoss)}%, Score ${latest.score}/100` },
                      { label: "Upload Speed", val: latest.upload, unit: "Mbps", icon: Upload, cls: ragUl(latest.upload), bg: "bg-emerald-50", prev: prev?.upload, higher: true, rgb: "168,85,247", ctx: `Download ${fmt(latest.download)}Mbps, Upload ${fmt(latest.upload)}Mbps, Latency ${fmt(latest.latency)}ms, Jitter ${fmt(latest.jitter)}ms, Packet Loss ${fmt(latest.packetLoss)}%, Score ${latest.score}/100` },
                      { label: "Latency", val: latest.latency, unit: "ms", icon: Timer, cls: latCls(latest.latency), bg: "bg-emerald-50", prev: prev?.latency, higher: false, rgb: "16,185,129", ctx: `Download ${fmt(latest.download)}Mbps, Upload ${fmt(latest.upload)}Mbps, Latency ${fmt(latest.latency)}ms, Jitter ${fmt(latest.jitter)}ms, Packet Loss ${fmt(latest.packetLoss)}%, Score ${latest.score}/100` },
                      { label: "Jitter", val: latest.jitter, unit: "ms", icon: Activity, cls: ragJitter(latest.jitter), bg: "bg-amber-50", prev: prev?.jitter, higher: false, rgb: "245,158,11", ctx: `Download ${fmt(latest.download)}Mbps, Upload ${fmt(latest.upload)}Mbps, Latency ${fmt(latest.latency)}ms, Jitter ${fmt(latest.jitter)}ms, Packet Loss ${fmt(latest.packetLoss)}%, Score ${latest.score}/100` },
                      { label: "Packet Loss", val: latest.packetLoss, unit: "%", icon: Wifi, cls: (latest.packetLoss ?? 0) === 0 ? "text-emerald-600" : (latest.packetLoss ?? 0) < 1 ? "text-amber-500" : "text-red-500", bg: "bg-indigo-50", prev: prev?.packetLoss, higher: false, rgb: "99,102,241", ctx: `Download ${fmt(latest.download)}Mbps, Upload ${fmt(latest.upload)}Mbps, Latency ${fmt(latest.latency)}ms, Jitter ${fmt(latest.jitter)}ms, Packet Loss ${fmt(latest.packetLoss)}%, Score ${latest.score}/100` },
